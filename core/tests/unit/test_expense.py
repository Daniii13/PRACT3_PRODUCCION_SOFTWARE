...
def test_negative_amount_raises_error():
    """
    Prueba que crear un objeto Expense con un valor negativo en el campo 'amount' (por
    ejemplo, -5) genera la excepción específica InvalidAmountError definida en
    domain_error.py.

    - Se espera que si se intenta instanciar un gasto con un monto negativo, el
      constructor de Expense detecte esta situación inválida y lance el error
      adecuado, impidiendo la creación del objeto.
    - Esta validación garantiza que no se puedan registrar gastos donde la cantidad
      gastada sea menor a cero, manteniendo la integridad del dominio de gastos.
...
def test_future_date_raises_error():
    ...
    with pytest.raises(InvalidExpenseDateError):
        Expense(
            id=1, title="Future", amount=10, description="", expense_date=future_date
        )
